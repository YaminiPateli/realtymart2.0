...
@Component({
  selector: 'app-ownerpropertyrent',
  templateUrl: './ownerpropertyrent.component.html',
  styleUrls: ['./ownerpropertyrent.component.css'],
})
export class OwnerpropertyrentComponent {
  tooltipVisible = false;
  tooltipPosition = { top: '0px', left: '0px' };
  @ViewChild('otpModel') otpModel!: ElementRef;
  @ViewChild('otpContactModel') otpContactModel!: ElementRef;
  private apiUrl: string = environment.apiUrl;
  ownerPropertyrentData: any;
  ...
  loadOwnerProperty() {
    ...
    this.http
      .get<any>(
        `${environment.apiUrl}isownerproperty/rent/${cityName}?page=${this.currentPage}`
      )
      .subscribe(
        (response) => {
  ...
  //   this.paginatedData = this.ownerlauchedproperty.slice(startIndex, endIndex);
  // }
