...
import { PropertyincitybuyService } from '../service/propertyincitybuy.service';
...
@Component({
  selector: 'app-propertyincitybuy',
...
export class PropertyincitybuyComponent {
  ...
  constructor(
    ...
    private PropertyincitybuyService: PropertyincitybuyService,
    ...
    // }, 0);
    // this.PropertyincitybuyService.getownerpropertybuy()?.subscribe((ownerPropertyData) => {
  ...
  loadProperties() {
    ...
    this.http
      .get<any>(
        `${environment.apiUrl}propertybuyincity/${this.cityget}?page=${this.currentPage}`
      )
      .subscribe(
        (response) => {
